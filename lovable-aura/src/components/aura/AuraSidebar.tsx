...
const items: { to: string; label: string; sub: string; persona: Persona; badge?: string; Icon: typeof Telescope }[] = [
  { to: "/",              label: "Executive Brief",    sub: "Miranda agent",           persona: "Executive", badge: "agent", Icon: Telescope },
  { to: "/communication", label: "Inbox Dispatch",     sub: "Brief delivery",          persona: "Executive", Icon: Mailbox },
  { to: "/operator",      label: "Operator Evidence",  sub: "Quant studio",            persona: "Operator",  Icon: Cpu },
  { to: "/demo",          label: "Architecture",       sub: "Models & workflow",       persona: "Operator",  badge: "ready", Icon: ClipboardCheck },
];

export const AuraSidebar = () => {
  const { pathname } = useLocation();
  const brandName = mieClient.config.brand;
  ...
  return (
    <aside className="w-[260px] shrink-0 hidden md:flex flex-col border-r border-border/60 glass">
      ...
      <div className="px-6 pt-7 pb-8">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            <div className="font-sans text-lg font-semibold text-obsidian">{brandName}</div>
            <div className="text-[9px] tracking-couture uppercase text-muted-foreground">{customerName}</div>
          </div>
        </div>
      </div>
          ...
          return (
            <div key={persona} className="pb-3">
              <div className="px-3 pt-2 pb-2 flex items-center gap-2">
                <span className={`h-1 w-3 rounded-full ${persona === "Executive" ? "bg-amethyst" : "bg-mint-deep"}`} />
                  ...
                  >
                    <span className={`h-9 w-9 rounded-md flex items-center justify-center transition-colors
                      ${active
                        ...
                        : persona === "Executive" ? "bg-secondary text-amethyst" : "bg-mint text-mint-deep"}`}>
                      <Icon className="h-4 w-4" strokeWidth={1.4} />
                    </span>
                    <span className="flex-1 text-left">
                      <span className="block text-sm font-medium font-sans flex items-center gap-2">
                        {label}
                        {badge && (
                    ...
                    {active && <span className="h-1.5 w-1.5 rounded-full bg-mint-deep halo-pulse" />}
                  </NavLink>
                );
              })}
            </div>
      ...
      <div className="mt-auto p-4">
        <div className="rounded-xl border-iridescent bg-gradient-mint p-4 shadow-glass">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] tracking-couture uppercase text-mint-deep">Polars Engine</span>
            </div>
          ...
          <div className="mt-3 h-1 w-full rounded-full bg-secondary overflow-hidden">
            <div className={`h-full ${ready ? "w-full bg-mint-deep" : "w-1/2 bg-amber-500"}`} />
          </div>
          <div className="mt-2 flex items-center justify-between font-mono text-[10px] text-muted-foreground">
            ...
            <span className="text-mint-deep">Nemotron Nano</span>
          </div>
        </div>
      </div>
    </aside>
